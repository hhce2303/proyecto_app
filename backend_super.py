...
def open_hybrid_events(username, session_id=None, station=None, root=None):
    ...
    # ⭐ VARIABLES GLOBALES PARA AUTOCOMPLETE INLINE
    autocomplete_widget = None  # Widget temporal de autocompletado
    autocomplete_data = {'row': None, 'col': None, 'values': [], 'processing': False}
    
    # ⭐ FUNCIÓN: Crear Entry con autocompletado inline
    def create_autocomplete_inline(row, col):
        """Crea un Entry con autocompletado sobre la celda seleccionada"""
        nonlocal autocomplete_widget
        
        # Evitar crear múltiples widgets
        if autocomplete_data['processing']:
            return
        
        autocomplete_data['processing'] = True
        
        try:
            # Destruir widget previo si existe
            if autocomplete_widget:
                try:
                    autocomplete_widget.destroy()
                except:
                    pass
                autocomplete_widget = None
            
            # Obtener valores según columna
            if col == 1:  # Sitio
                autocomplete_data['values'] = under_super.get_sites()
            elif col == 2:  # Actividad
                autocomplete_data['values'] = under_super.get_activities()
            else:
                autocomplete_data['processing'] = False
                return
            
            autocomplete_data['row'] = row
            autocomplete_data['col'] = col
            
            # Obtener valor actual de la celda
            current_value = sheet.get_cell_data(row, col) or ""
            
            # Obtener posición en pantalla de la celda
            try:
                bbox = sheet.get_cell_coords(row, col)
                if not bbox:
                    autocomplete_data['processing'] = False
                    return
            ...
            # Crear Entry con AutoCompleteEntry
            from autocomplete_entry import AutoCompleteEntry
            
            # Frame contenedor para el entry (posicionado sobre la celda)
            autocomplete_frame = tk.Frame(sheet, bg="#2b2b2b", highlightthickness=2, 
                                         highlightbackground="#4a90e2")
            autocomplete_frame.place(x=cell_x, y=cell_y, width=cell_width, height=cell_height)
            ...
            autocomplete_entry = AutoCompleteEntry(
                ...
                values=autocomplete_data['values'],
                font=("Segoe UI", 10),
                bg="#2b2b2b",
                fg="#ffffff",
            ...
            autocomplete_entry.pack(fill="both", expand=True)
            
            # ⭐ CRÍTICO: Forzar actualización del layout antes de enfocar
            # Esto asegura que el Entry tenga las coordenadas correctas para el listbox
            autocomplete_frame.update_idletasks()
            
            autocomplete_entry.focus_set()
            autocomplete_entry.select_range(0, tk.END)
            
            # ⭐ HACK: Forzar que el listbox use coordenadas absolutas correctas
            # Sobrescribir el método _show_suggestions temporalmente
            original_show_suggestions = autocomplete_entry._show_suggestions
            
            def patched_show_suggestions(matches):
                """Versión parchada que usa coordenadas absolutas del sheet"""
                # Crear listbox si no existe
                if autocomplete_entry.listbox is None:
                    # ⭐ Crear listbox en la ventana top-level en lugar del frame
                    autocomplete_entry.listbox = tk.Listbox(
                        top,  # Usar ventana principal en lugar del frame
                        height=min(8, len(matches)),
                        bg="#2b2b2b",
                        fg="#ffffff",
                        selectbackground="#4a90e2",
                        selectforeground="#ffffff",
                        font=autocomplete_entry.cget("font"),
                        relief="solid",
                        borderwidth=1
                    )
                    
                    # Evento de selección con click
                    autocomplete_entry.listbox.bind("<Button-1>", autocomplete_entry._on_listbox_click)
                    autocomplete_entry.listbox.bind("<Double-Button-1>", autocomplete_entry._on_listbox_double_click)
                
                # Actualizar contenido
                autocomplete_entry.listbox.delete(0, tk.END)
                for match in matches:
                    autocomplete_entry.listbox.insert(tk.END, match)
                
                # ⭐ Posicionar usando coordenadas ABSOLUTAS (rootx/rooty)
                try:
                    x = autocomplete_entry.winfo_rootx() - top.winfo_rootx()
                    y = autocomplete_entry.winfo_rooty() - top.winfo_rooty() + autocomplete_entry.winfo_height()
                    width = autocomplete_entry.winfo_width()
                    
                    autocomplete_entry.listbox.place(x=x, y=y, width=width)
                    autocomplete_entry.listbox.config(height=min(8, len(matches)))
                    autocomplete_entry.listbox.lift()  # Traer al frente
                    autocomplete_entry.listbox_visible = True
                except Exception as e:
                    print(f"[DEBUG] Error positioning listbox: {e}")
            
            autocomplete_entry._show_suggestions = patched_show_suggestions
            
            autocomplete_widget = autocomplete_frame
            
            def apply_autocomplete(e=None):
                """Aplicar autocompletado y guardar"""
                nonlocal autocomplete_widget
                
                # ⭐ Obtener valor del Entry (ya procesado por AutoCompleteEntry)
                ...
                print(f"[DEBUG] apply_autocomplete - typed_value: '{typed_value}'")
                
                # ⭐ El AutoCompleteEntry ya hizo el matching al presionar Enter
                # Solo necesitamos tomar el valor y guardarlo
                best_match = typed_value
                
                # Si es columna Sitio y el valor no tiene formato "Nombre (ID)", 
                # intentar buscar una coincidencia mejorada
                if autocomplete_data['col'] == 1 and "(" not in typed_value:
                    # Algoritmo de búsqueda mejorado como fallback
                    typed_lower = typed_value.lower()
                    
                    # 1. Si es solo números, buscar por ID
                    if typed_value.isdigit():
                        search_id = typed_value
                        for value in autocomplete_data['values']:
                            if f"({search_id})" in value:
                                best_match = value
                                print(f"[DEBUG] Encontrado por ID: {best_match}")
                                break
                    
                    # 2. Buscar coincidencia exacta
                    if best_match == typed_value:
                        for value in autocomplete_data['values']:
                            if value.lower() == typed_lower:
                                best_match = value
                                print(f"[DEBUG] Encontrado exacto: {best_match}")
                                break
                    
                    # 3. Buscar que empiece con
                    if best_match == typed_value:
                        for value in autocomplete_data['values']:
                            if value.lower().startswith(typed_lower):
                                best_match = value
                                print(f"[DEBUG] Encontrado startswith: {best_match}")
                                break
                    
                    # 4. Buscar que contenga
                    if best_match == typed_value:
                        for value in autocomplete_data['values']:
                            if typed_lower in value.lower():
                                best_match = value
                                print(f"[DEBUG] Encontrado contains: {best_match}")
                                break
                    
                    # 5. Búsqueda por palabras
                    if best_match == typed_value:
                        words = typed_lower.split()
                        for value in autocomplete_data['values']:
                            value_lower = value.lower()
                            if all(word in value_lower for word in words):
                                best_match = value
                                print(f"[DEBUG] Encontrado por palabras: {best_match}")
                                break
                
                print(f"[DEBUG] Actualizando celda [{autocomplete_data['row']}, {autocomplete_data['col']}] = '{best_match}'")
                
                # Actualizar la celda con el mejor match
                sheet.set_cell_data(autocomplete_data['row'], autocomplete_data['col'], best_match)
                
                # ⭐ Refrescar inmediatamente para mostrar el cambio
                sheet.refresh()
                
                # ⭐ Guardar automáticamente DESPUÉS de un delay más largo
                # para asegurar que la celda se actualizó
                top.after(500, auto_save_pending)
                
                cleanup_autocomplete()
            
            def cleanup_autocomplete():
                """Limpiar widget de autocompletado"""
                nonlocal autocomplete_widget
            ...
            # ⭐ Reemplazar binding de Enter para guardar después de AutoComplete
            def custom_on_enter(e):
                """Enter: permitir AutoCompleteEntry procesar primero, luego guardar"""
                # ⭐ IMPORTANTE: No interferir con el proceso del AutoCompleteEntry
                # El _on_enter del AutoCompleteEntry ya se ejecutó y actualizó el Entry
                # Ahora esperamos un momento para que el valor esté disponible
                
                # Esperamos 200ms para asegurar que AutoCompleteEntry terminó de actualizar
                top.after(200, apply_autocomplete)
                
                # NO hacer "break" aquí - dejar que el evento se propague normalmente
                # para que el AutoCompleteEntry lo procese primero
            
            # ⭐ Vincular DESPUÉS del binding original del AutoCompleteEntry
            # Usamos add=True para no sobrescribir el binding original
            autocomplete_entry.bind("<Return>", custom_on_enter, add=True)
            autocomplete_entry.bind("<KP_Enter>", custom_on_enter, add=True)
            autocomplete_entry.bind("<Escape>", lambda e: cleanup_autocomplete())
            
            # ⭐ Al perder foco, aplicar y guardar
            def on_entry_focus_out(e):
                """Al perder foco, guardar si hay cambios"""
                # Solo si el widget aún existe
                if autocomplete_widget:
                    apply_autocomplete()
            
            autocomplete_entry.bind("<FocusOut>", on_entry_focus_out, add=True)
            
        except Exception as e:
            print(f"[DEBUG] create_autocomplete_inline error: {e}")
            import traceback
            traceback.print_exc()
            autocomplete_data['processing'] = False
